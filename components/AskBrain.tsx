...
export const AskBrain: React.FC<AskBrainProps> = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const answer = await apiService.askBrain(query);
      const aiMsg: Message = { id: crypto.randomUUID(), role: 'assistant', content: answer };
      setMessages(prev => [...prev, aiMsg]);
    } catch (e) {
      setMessages(prev => [...prev, { id: crypto.randomUUID(), role: 'assistant', content: "Sorry, I encountered an error accessing your brain. Is the backend running?" }]);
    } finally {
      setIsLoading(false);
    }
  };
